...
class MatchAPI extends AbstractAPI {
	...
	inviteToMatch(context, match_id, caller_gamer_id, invitee_id, eventOsn){
		...
		.then(match=> {
			...
			const alreadyInvited = ((() => {
				const result = [];
				for (p of Array.from(match.players)) { 					if (p.equals(invitee_id)) {
						result.push(0);
					}
				}
				return result;
			...
			const alreadyPart = ((() => {
				const result1 = [];
				for (p of Array.from(match.invitees || [])) { 					if (p.equals(invitee_id)) {
						result1.push(0);
					}
				}
				return result1;
	...
	joinMatch(context, match_id, gamer_id, eventOsn){
		...
		.then(match=> {
			...
			if (((() => {
				const result = [];
				for (let p of Array.from(match.players)) { 					if (gamer_id.equals(p)) {
						result.push(p);
					}
				}
				return result;
