...
class ConnectAPI extends AbstractAPI {
	...
	configure(xtralifeapi, callback){
		...
		this.facebookValidTokenAsync = Q.promisify(facebook.validToken, {context: facebook});
		this.googleValidTokenAsync = Q.promisify(google.validToken, {context: google});
	...
	convertAccountToEmail(user_id, email, sha_password){
		if (!/^[^@ ]+@[^\.@ ]+\.[^@ ]+$/.test(email)) { return Q.reject(new errors.BadArgument); }
		return this._checkAccountForConversion("email", user_id, email)
		.then(() => {
			const modification = { $set: {
				network: "email",
				networkid: email,
