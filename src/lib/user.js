...
class UserAPI extends AbstractAPI {
	...
	_checktype(value){
		switch (typeof value) {
			case "number": case "string": case "boolean":
				...
				break;
			case "object":
				if (!Array.isArray(value)) { return new errors.BadPropertyType; }
				for (let elem of Array.from(value)) { 
					if (!["number", "string", "boolean"].includes(typeof(elem))) { return new errors.BadPropertyType; }
				}
				break;
		}
		return null;
	...
	write(context, domain, user_id, key, value){
		...
		if (key != null) {
			if (value == null) { return (() => { throw new errors.MissingPropertyValue; })(); }
		...
		return {
			account: {
				...
				changeEmail: (user_id, email)=> {
					const changeAsync = Q.promisify(this.xtralifeapi.connect.changeEmail, {context: this.xtralifeapi.connect});
			...
			relations: {
				...
				setFriendStatus: (domain, user_id, friend_id, status, osn)=> {
					if (this.xtralifeapi.game.checkDomainSync(context.game.appid, domain)) {
						const setFriendStatus = Q.promisify(this.xtralifeapi.social.setFriendStatus, {context: this.xtralifeapi.social});
				...
				godfather: {
					set: (domain, user_id, godfather, options)=> {
						if (this.xtralifeapi.game.checkDomainSync(context.game.appid, domain)) {
							const asyncFn = Q.promisify(this.xtralifeapi.social.setGodfather, {context: this.xtralifeapi.social});
					...
					get: (domain, user_id)=> {
						if (this.xtralifeapi.game.checkDomainSync(context.game.appid, domain)) {
							const asyncFn = Q.promisify(this.xtralifeapi.social.getGodfather, {context: this.xtralifeapi.social});
					...
					getCode: (domain, user_id)=> {
						if (this.xtralifeapi.game.checkDomainSync(context.game.appid, domain)) {
							const asyncFn = Q.promisify(this.xtralifeapi.social.godfatherCode, {context: this.xtralifeapi.social});
					...
					getChildren: (domain, user_id)=> {
						if (this.xtralifeapi.game.checkDomainSync(context.game.appid, domain)) {
							const asyncFn = Q.promisify(this.xtralifeapi.social.getGodchildren, {context: this.xtralifeapi.social});
							return asyncFn(context, domain, user_id);
