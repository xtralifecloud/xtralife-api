...
describe.skip("test then catch then", function() {
	...
	return it("shoud let me catch and continue", () => Q.resolve(null).then(function(){
        throw new Error("thrown");}).catch(function(err){
        err.message.should.eql('thrown');
        return 'continues';}).then(function(result){
        result.should.eql('continues');
        throw new Error("thrown2");}).catch(function(err){
        err.message.should.eql('thrown2');
        return 'continues2';}).then(result => result.should.eql('continues2')));
});
